...
def main():
    ...
    parser.add_argument('--img-name', type=str, required=True, help='Name of the input image (e.g. 0002_c002_00030600_0.jpg for VeRi776, MVI_20011_002_img00010.jpg for VRIC)')
    ...
    for i, path in enumerate(query_img_paths):
        if os.path.basename(path) == args.img_name:
            query_idx = i
            break
    ...
    if query_idx == -1:
        print(f"Image {args.img_name} not found in {cfg.dataset.name} dataset.")
        return
        
    print(f"Found {args.img_name} in {'query' if is_from_query else 'gallery'} dataset.")
    ...
    vis_path = os.path.join(test_result_dir, f"{args.img_name}_retrieval.png")
